...
public class UserService {
    ...
    public Optional<UserEntity> authenticate(String username, String password) {
        ...
        try {
            ...
            if (passwordMatch) {
                // Auto-upgrade legacy hash (SHA-256 or plain text) to BCrypt
                if (PasswordUtil.isLegacyHash(storedPassword)) {
                    String newBcryptHash = PasswordUtil.hashPassword(password);
                    user.setPassword(newBcryptHash);
                    userDAO.update(user);
                    LoggerUtil.logInfo(UserService.class,
                            "Password automatically upgraded to BCrypt for user: " + username);
                }
     ...
     *
     * Note: Super-admin (Bilawal/breakthewall) is hardcoded in Login.java, NOT in
     * database
     *
